...
class y_quantize(nn.Module):
    ...
    def forward(self, image):
        image = image.float() / (self.y_table.to(image) * self.factor)
...
class c_quantize(nn.Module):
    ...
    def __init__(self, rounding, factor=1):
        ...
        self.rounding = rounding
        self.factor = factor
        self.register_buffer('c_table',c_table)

    def forward(self, image):
        image = image.float() / (self.c_table.to(image) * self.factor)
...
class y_dequantize(nn.Module):
    ...
    def forward(self, image):
        return image * (self.y_table.to(image) * self.factor)
...
class c_dequantize(nn.Module):
    ...
    def forward(self, image):
        return image * (self.c_table.to(image) * self.factor)
...
class idct_8x8(nn.Module):
    ...
    def forward(self, image):
        image = image * self.alpha.to(image)
        result = 0.25 * torch.tensordot(image, self.tensor.to(image), dims=2) + 128
...
class ycbcr_to_rgb_jpeg(nn.Module):
    ...
    def forward(self, image):
        result = torch.tensordot(image + self.shift.to(image), self.matrix.to(image), dims=1)
...
# if x is near 1, the above is x
# if x is near 0, the above is x**3
...
class DiffJPEG(nn.Module):
    def __init__(self, quality=75, differentiable=True, height=256, width=256,rounding=diff_round):
        ...
        self.name = "DiffJPEG"+str(quality)
        # image_r = image * 255
